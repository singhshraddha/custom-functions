...
class SS_DataQualityChecks_2(BaseComplexAggregator):
    """
    Data Quality module will help assess the quality of incoming sensor data, using the provided metrics.
    ...
    def __init__(self, source=None, quality_checks_str=[], quality_checks_num=[], quality_checks_bool=[],
                 name=None):
        super().__init__()

        self.input_items = source
        self.quality_checks = np.concatenate((quality_checks_str, quality_checks_num, quality_checks_bool))
        self.output_items = name
        logger.debug(f'Data Quality Checks will be performed for : {source}')
